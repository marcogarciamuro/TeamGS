...
LEAGUES = [
    {"name": "La Liga", "formatted_name": "la%20liga", "id": LA_LIGA_ID},
    {
        "name": "Premier League",
        "formatted_name": "premier%20league",
        "id": PREMIER_LEAGUE_ID,
    },
    {"name": "Serie A", "formatted_name": "serie%20a", "id": SERIE_A_ID},
    {"name": "Bundesliga", "formatted_name": "bundesliga", "id": BUNDESLIGA_ID},
    {"name": "Ligue 1", "formatted_name": "ligue%201", "id": LIGUE_1_ID},
]
...
NEWS_API_HEADERS = {"User-agent": "Mozilla/5.0"}
...
# returns team autocomplete suggestions from team search
...
def call_api(endpoint):
    ...
    try:
        ...
        if results == 0:
            return None
    ## if api did not return results due to limit
    except:
        return None
...
def get_player_details(player_id):
    endpoint = "/v3/players?id=" + str(player_id) + "&season=" + str(cur_season)
    print(endpoint)
    ...
    if nationality is None:
        print("PLAYER HAS NO NATIONALITY")
    ...
    if player_height:
        ...
        height_in = round(float("0." + str(height_cms / 30.48).split(".")[-1]) * 12)
    else:
        height_ft = None
        height_in = None
...
def get_player_best_name(general_name, short_name, firstname, lastname):
    print("general name: " + general_name)
    ...
    if Player.objects.filter(team=team_obj).exists():
        ...
        for player in player_list:
            if not player["number"]:
                continue
            position = shorten_player_position(player["position"])
            print("CURRENT PLAYER ID:", player["id"])
            # player_details = get_player_details(player['id'])
            player_name = player["name"]
            # This function sets player's most commonly known name, but uses too many api calls
            ...
            Player.objects.create(
                ...
                position=position,
                photo=player["photo"],
                # nationality=player['nationality'], not obtainable unless A LOT of api calls are made
                # height_feet=player['height_ft'], not obtainable unless A LOT of api calls are made
                # height_inches=player['height_in'], not obtainable unless A LOT of api calls are made
                # weight=player['weight'], not obtainable unless A LOT of api calls are made
            )
        players = Player.objects.filter(team=team_obj)
        return players


# retrieves team object based on team search to send to template to render widget with team's matches
...
def team_page(request, team_name=None):
    ...
    if request.method == "POST":
        ...
        if search_form.is_valid():  # process form data which is the searched team name
            ...
            if team is None:
                team_not_found_url = (
                    "/soccer/team-not-found?team=" + team_search.replace(" ", "+")
                )
...
def create_team(team_details):
    ...
    Team.objects.create(
        name=team_name, teamID=team_id, league=team_league, logo=team_logo
    )
...
def get_team_articles_from_api(team):
    ...
    #     "&apiKey=" + KEY
    endpoint = (
        "/v2/everything?sortBy=publishedAt&language=en&searchIn=title&q="
        + q
        + "&apiKey="
        + KEY
    )
...
def get_league_articles_from_api(league_obj):
    ...
    endpoint = (
        "/v2/everything?sortBy=publishedAt&language=en&q="
...
def get_team_articles(team):
    ...
    cached_articles = TeamArticle.objects.filter(team=team).order_by("-retrieval_date")
...
def get_league_articles():
    one_day_ago = timezone.now() - timedelta(days=1)
    LEAGUE_IDS = [PREMIER_LEAGUE_ID, LA_LIGA_ID, SERIE_A_ID, BUNDESLIGA_ID, LIGUE_1_ID]
    ...
    if not cached_articles.exists():
        ...
        if cached_articles[0].retrieval_date <= one_day_ago:
            ...
            print("Serving CACHED ARTICLES")
            premier_league_obj = League.objects.get(league_id=PREMIER_LEAGUE_ID)
            ...
            cached_premier_league_article = LeagueArticle.objects.filter(
                league=premier_league_obj
            ).order_by("-retrieval_date")[:1]
            ...
            cached_bundesliga_article = LeagueArticle.objects.filter(
                league=bundesliga_obj
            ).order_by("-retrieval_date")[:1]
            cached_ligue_1_article = LeagueArticle.objects.filter(
                league=ligue_1_obj
            ).order_by("-retrieval_date")[:1]
            articles = (
                list(cached_premier_league_article)
                + list(cached_la_liga_article)
                + list(cached_serie_a_article)
                + list(cached_bundesliga_article)
                + list(cached_ligue_1_article)
            )
    return articles
